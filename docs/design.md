...
并行评审时，Dispatcher 同时启动多个 subprocess，`sync.WaitGroup` 并发等待全部完成后合并返回。
...
| Tool 名称 | 描述 | 参数 |
|-----------|------|------|
| `quorum_review` | 将内容发送给 OpenCode 后端进行独立评审 | `content`, `context`, `backend`, `file_path`, `timeout` |
...
```json
{
  ...
  "properties": {
    "content": {
      "type": "string",
      "description": "待评审内容（代码、设计文档等），或评审指令（如「评审最近的 git 变更」）。OpenCode 会自行读取文件、执行命令"
    },
    "context": {
      "type": "string",
    ...
    "backend": {
      "type": "string",
      "default": "all",
      "description": "评审后端：配置文件中的后端名称（如 GLM-5、MiniMax-M2.5），或 all 并行调用所有后端"
    },
    "file_path": {
      "type": "string",
      "description": "文件路径（可选），作为评审上下文提示"
    },
    "timeout": {
      "type": "number",
      "description": "超时时间（秒）。覆盖 config 中的默认值。复杂评审建议设置 600-900"
    }
  },
  "required": ["content"]
}
...
`{{.Content}}` 可以是代码片段、设计文档，也可以是评审指令（如"评审最近的 git 变更"）。OpenCode 作为 agent 会自行读取文件、执行命令。
...
```
quorum-cc serve         # 启动 MCP Server（stdio 模式，由 Claude Code 自动调用）
...
│   │   └── config_test.go
│   └── cli/
│       └── cli.go               # CLI 命令实现 (init/status/test)
├── docs/
│   └── design.md                # 设计文档
...
升级时如果配置格式变更，`quorum-cc serve` 启动时自动迁移（读取旧格式，写入新格式，备份旧文件为 `config.yaml.bak`）。MVP 阶段暂未实现自动迁移，`version` 字段预留供后续使用。
...
`content` 参数超过 100KB 时自动截断，防止超长内容导致 OpenCode 后端 OOM 或超时。截断后附加提示信息。

### 9.2 Prompt 注入

`content` 参数来自 Claude Code（而非直接来自用户输入），风险较低。但 OpenCode 后端作为 agent 具有文件读写和命令执行能力，恶意指令理论上可能被执行。当前 MVP 不做额外防护，依赖 OpenCode 自身的安全机制。

### 9.3 凭证隔离

quorum-cc 不管理任何 API key。OpenCode 的凭证由 OpenCode 自身管理（`opencode auth`），quorum-cc 的子进程继承当前用户环境。配置文件 `config.yaml` 中不存储敏感信息。

### 9.4 错误信息

错误信息可能包含文件路径、模型名称等信息，但不包含 API key 或用户凭证。错误信息通过 MCP 协议返回给 Claude Code，不会暴露给外部。
...
**修订记录**：
- v1.6: 第二轮 review 修复 — Prompt 模板描述对齐实现（content 泛化）；新增安全性章节（§9）；内容长度限制 100KB 自动截断；MCP Server 版本号从 build 注入；后端名示例统一大小写
